...
class User {
    ...
    getMacros() {
        ...
        const dailyCals = this.getTargetCals(); 

        const leanBodyMass = this.weight*(1-this.bodyFat/100); 

        const proteinG = Math.floor(leanBodyMass*1.5); 

        const fatG = Math.floor(dailyCals*0.2/9);

        const carbsG = Math.floor((dailyCals - 9*fatG - 4*proteinG)/4);
